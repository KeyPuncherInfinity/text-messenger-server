...
class Messenger {
    ...
    async acceptRequest(f_uid, s_uid) {
        var l_uid = f_uid < s_uid ? f_uid : s_uid;
        var r_uid = f_uid > s_uid ? f_uid : s_uid;
        var result = await db.changeChangeClientRelation(l_uid, r_uid);

        return result;
    }

    async declineRequest(f_uid, s_uid) {
        var l_uid = f_uid < s_uid ? f_uid : s_uid;
        var r_uid = f_uid > s_uid ? f_uid : s_uid;
        var result = await db.deleteClientRelation(l_uid, r_uid);
