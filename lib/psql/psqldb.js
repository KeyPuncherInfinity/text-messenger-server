...
class db {
    ...
    async changeClientRelation(l_uid, r_uid) {
        try {
            const result = await conn.client.query(`UPDATE tm_users_relation_status SET status='mutual' WHERE l_uid='${l_uid}' AND r_uid='${r_uid}'`);
            if (result.rowCount == 1) {
                return {code: 1};
            } else {
                return {code: 2};
    ...
    async deleteClientRelation(l_uid, r_uid) {
        try {
            const result = await conn.client.query(`DELETE FROM tm_users_relation_status WHERE l_uid='${l_uid}' AND r_uid='${r_uid}'`);
            if (result.rowCount == 1) {
                return {code: 1};
            } else {
                return {code: 2};
